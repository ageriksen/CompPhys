...
)
{
    ...
    vector<double> minima(3);
    minima[0] = m_parameters[0][index];
    minima[1] = alpha0;
    setWaveFunction( WF );
    minima[2] = beta0( minima[0], m_MCCyclesFull );
    m_stepLength = stepFinder(minima);
...
{ // no change in minimal alpha per omega. Set it to 1.0
    ...
    cout << "length of param: " << param.size() << endl;
    for( auto element: param )
    {
        cout << element << endl;
    }

    double delta = m_parameters[1].back();
    double energyMin = 1e6;
    double beta0 = 0;
...
double VMC::findAlpha( vector<double> param)
{
    ...
    double epsilon = 1;
    double ratio=epsilon*2;// ensuring ratio is greater than epsilon initially
    double energyOld = 0;
