...
)
{
    ...
    {
        cout << "omega = " << parameters[0][omega] << endl;
        cout << "vector has length " << parameters[0].size() << endl;
        delta = parameters[1][omega];
        cout << "and an accompanying stepLength " << delta << endl;
        varianceMin = 10; // pretty sure the variance should dip WAY below this.
        tmpAlpha = 0;
        tmpLine = "";
        cout << "jumping into alphas. " << endl;
        for( unsigned int alpha = 0; alpha < parameters[2].size(); alpha++ )
        {
            cout << " alpha: " << parameters[2][alpha] << endl;
            param[0] = parameters[0][omega];
            param[1] = parameters[2][alpha];
            for( auto out: param )
            {
                cout << "parameter: " << out << endl;
            }
